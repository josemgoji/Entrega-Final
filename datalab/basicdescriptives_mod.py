...
def info(df):
    "info about df. input: dataframe. output: # of columns, rows, unique rows..."   
    
    # Print the number of columns in the DataFrame
    print("Number of Columns:", len(df.columns))
    print('Number of total rows:', len(df))
    
    # Count of unique rows (considering all columns)
    unique_rows = df.drop_duplicates()       #drop duplicates and storage in unique_rows
    ...
    info_df = pd.DataFrame({
        ...
        'Non null values': [df[col].notnull().sum() for col in df.columns],
